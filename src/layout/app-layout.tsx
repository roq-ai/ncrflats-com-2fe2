...
import { getBuyers } from 'apiSdk/buyers';
...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['me', 'customers'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<BuyerInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/buyers` : null),
    () => getBuyers({ tenant_id: session?.user?.tenantId }).then(({ data }) => data),
  );
  loading = isLoading;
  ...
  const MockedLinkItems: Array<NavItemPropsInterface> = [
    ...
    {
      name: 'Buyers',
      path: isTenantUser ? `/buyers/view/${data?.[0]?.id}` : '/buyers',
      entity: 'buyer',
    ...
    { name: 'Sellers', path: '/sellers', entity: 'seller', service: AccessServiceEnum.PROJECT, icon: FiDollarSign },
