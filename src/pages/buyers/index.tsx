...
import { getBuyers, deleteBuyerById } from 'apiSdk/buyers';
import { BuyerInterface } from 'interfaces/buyer';

type ColumnType = ColumnDef<BuyerInterface, unknown>;
...
export function BuyerListPage(props: BuyerListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<BuyerInterface>>(
    () => `/buyers?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: BuyerInterface) => {
    if (hasAccess('buyer', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/buyers/view/${row.id}`);
    }
  };
...

    {
      ...
      cell: ({ row: { original: record } }: any) => (
        <Flex justifyContent="flex-end">
          <NextLink href={`/buyers/view/${record.id}`} passHref legacyBehavior>
            <Button
          ...
          {hasAccess('buyer', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
            <NextLink href={`/buyers/edit/${record.id}`} passHref legacyBehavior>
              <Button
                onClick={(e) => e.stopPropagation()}
                mr={2}
                padding="0rem 0.5rem"
          ...
          {hasAccess('buyer', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
            <IconButton
              onClick={(e) => {
                e.stopPropagation();
  ...
  return (
    <Box p={4} rounded="md" shadow="none">
      <AccessInfo entity="buyer" />
      <Flex justifyContent="space-between" mb={4}>
        <Text as="h1" fontSize="1.875rem" fontWeight="bold" color="base.content" {...titleProps}>
          Buyers
        </Text>
        {hasAccess('buyer', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/buyers/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
